  ...
  return (
    <section className="glass-strong p-5">
      <div className="flex items-center gap-2 mb-3">
        ...
        <h3 className="font-display text-lg">Share your Oakmont Digital Markets Groups experience</h3>
      </div>
      <p className="text-xs text-white/55 mb-3">Eligible after your first investment or deposit clears. Your testimonial may appear publicly on the Oakmont Digital Markets Groups landing page.</p>
      <form onSubmit={submit} className="space-y-2">
        <textarea required minLength={20} maxLength={600} value={text} onChange={(e) => setText(e.target.value)} placeholder="What stands out about trading and investing on Oakmont Digital Markets Groups?" className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-sm outline-none focus:border-accent-success/40 min-h-[90px]"/>
  ...
  return (
    <section className="glass-strong p-5 border border-accent-success/20">
      ...
      <p className="text-xs text-white/55 mb-4">
        Welcome to Oakmont Digital Markets Groups. Complete these three steps to unlock the full broker dashboard.
      </p>
...
export function SupportPanel() {
  ...
  return (
    <section className="glass-strong p-5">
      ...
      <p className="text-xs text-white/55 mt-1">
        Ask the Oakmont Digital Markets Groups desk anything - KYC, deposits, withdrawals, trade issues.
      </p>
      {showForm && (
                        ...
                        <li key={m.id} className={`rounded-lg px-3 py-2 text-xs whitespace-pre-wrap break-words ${m.authorRole === 'staff' ? 'bg-accent-success/10 border border-accent-success/20 text-white' : 'bg-white/5 border border-white/10 text-white/85'}`}>
                          <div className="flex items-center justify-between mb-0.5 text-[10px]">
                            <span className={m.authorRole === 'staff' ? 'text-accent-success' : 'text-white/55'}>
                              {m.authorRole === 'staff' ? 'Oakmont Digital Markets Groups support' : 'You'}
                            </span>
