...
const curatedTestimonials = [
    {
        ...
        rating: 5,
        text: 'Oakmont Digital Markets Groups gave me the confidence to move from scattered exchange balances into a more disciplined investment workflow. I can follow my allocation, review account activity, and understand what is happening across each strategy before making a decision.',
    },
    {
    ...
    },
    {
        ...
        rating: 5,
        text: 'I use Oakmont Digital Markets Groups to monitor market opportunities and keep my decisions organised. The experience makes it easy to separate browsing public market data from actual account activity once I sign in.',
    },
    {
    ...
    },
    {
        ...
        rating: 5,
        text: 'Oakmont Digital Markets Groups gives the platform a premium feel while keeping the workflow straightforward. The reporting, account controls, and market access make crypto investing feel more organised and accountable.',
    },
];
        ...
        <div className="mt-10 grid md:grid-cols-3 gap-3 sm:gap-4">
          {displayItems.map((t, i) => (<motion.div key={(t.id || t.name) + i} initial={{ opacity: 0, y: 14 }} whileInView={{ opacity: 1, y: 0 }} whileHover={{ y: -4 }} viewport={{ once: true }} transition={{ duration: 0.45, delay: i * 0.06, ease: [0.22, 1, 0.36, 1] }} className={i === 0 ? 'glass p-5 sm:p-6 md:col-span-2 bg-white/[0.04]' : 'glass p-5 sm:p-6'}>
                ...
                <div>
                  ...
                  <p className="text-xs text-white/55">{t.role || 'Oakmont Digital Markets Groups investor'}</p>
                </div>
