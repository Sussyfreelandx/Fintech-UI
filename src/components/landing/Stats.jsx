...
export function Stats() {
    ...
    return (<section className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-12 sm:py-14 lg:py-16">
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        {stats.map((s, i) => {
            const Icon = s.icon;
            return (<motion.div key={s.label} initial={{ opacity: 0, y: 12 }} whileInView={{ opacity: 1, y: 0 }} whileHover={{ y: -4 }} viewport={{ once: true, margin: '-80px' }} transition={{ duration: 0.45, delay: i * 0.05, ease: [0.22, 1, 0.36, 1] }} className="glass p-5 sm:p-6">
              <div className="flex items-center justify-between">
                <Icon className={`h-5 w-5 ${s.accent}`}/>
                <span className="text-xs text-white/40">Live</span>
              </div>
              <p className="mt-5 text-2xl sm:text-3xl font-display tracking-tight leading-none">{s.value}</p>
