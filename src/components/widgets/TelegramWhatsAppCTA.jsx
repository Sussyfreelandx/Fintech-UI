...
export function TelegramWhatsAppCTA() {
    const telegramUrl = process.env.NEXT_PUBLIC_TELEGRAM_SUPPORT_URL || 'https://t.me/Oakmont Digital Capital GroupSupport';
    const whatsappUrl = process.env.NEXT_PUBLIC_WHATSAPP_SUPPORT_URL || 'https://wa.me/15555550123';
    return (<motion.div drag dragMomentum={false} dragElastic={0.08} whileDrag={{ scale: 1.03 }} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="fixed bottom-20 lg:bottom-5 right-4 z-[60] flex flex-col items-end gap-2 touch-none cursor-grab active:cursor-grabbing" aria-label="Draggable support shortcuts">
