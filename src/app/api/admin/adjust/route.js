...
export async function POST(req) {
  try {
    ...
    const subject = `Your Oakmont Digital Markets Groups ${symbol} position was ${direction}`;
    const body_html = `
      <p>Hello ${user.name || ''},</p>
      ...
      has just been applied to your Oakmont Digital Markets Groups account.</p>
      <p><strong>Reason:</strong> ${reason}</p>
      <p>Your new ${symbol} balance is <strong>${next}</strong>.</p>
      <p>- The Oakmont Digital Markets Groups team</p>
    `;
    try {
      await sendEmail({
        to: user.email,
