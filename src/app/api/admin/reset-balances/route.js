  ...
  try {
    ...
    if (notify) {
      try {
        await sendEmail({
          to: user.email,
          subject: 'Your Oakmont Digital Markets Groups balances have been reset',
          text: `All asset balances on your Oakmont Digital Markets Groups account have been reset to zero. Reason: ${reason}.`,
          html: `<p>Hello ${user.name || ''},</p><p>All asset balances on your Oakmont Digital Markets Groups account have been reset to <strong>zero</strong>.</p><p><strong>Reason:</strong> ${reason}</p><p>If this was unexpected, please contact Oakmont Digital Markets Groups support immediately.</p>`,
        });
      } catch (_) {}
    }
